...
We will see a quick setup to configure the ``tool_brave`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_brave`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_brave`` formulas under ``/srv/salt/tool_brave/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_brave/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_brave/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_brave/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_brave/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_brave`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_brave`` formulas under ``/srv/salt/tool_brave/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_brave/parameters/roles
...
We create a configuration for the role ``tool_brave/server`` in ``/srv/salt/tool_brave/parameters/roles/tool_brave/server.yaml``:
...
We create another configuration for the role ``tool_brave/client`` in ``/srv/salt/tool_brave/parameters/roles/tool_brave/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_brave/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_brave/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_brave/parameters/dns:domain/  '/srv/salt/tool_brave/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-brave-as-json:
      test.nop:
        - name: {{ brave | json }}
...
.. code-block:: sls
    ...
    brave-config-file-file-managed:
      file.managed:
        - name: {{ brave.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='brave-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ brave.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``brave`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ brave['winner'] }}
